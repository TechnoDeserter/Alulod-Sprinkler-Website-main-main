...
export default abstract class DH {
  ...
  static getDateWithoutTime(date: Date) {
    ...
    return `${month < 10 ? "0" + month : month}/${day < 10 ? "0" + day : day}/${String(year).substring(2)
      }`;
  }
  ...
  // January 30, 2025
  static fromDateToFullDate(date: Date = new Date()) {
    const month = this.monthFull[date.getMonth()];
    const day = date.getDate();
    const year = date.getFullYear();
  ...
  // 10:30 PM
  static fromDateToFullTime(date: Date = new Date()) {
    let hour = date.getHours();
    const minute = date.getMinutes();
    const period = hour >= 12 ? "PM" : "AM";
    hour = hour % 12 || 12; // convert to 12-hour format
